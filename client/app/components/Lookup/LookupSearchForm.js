...
class LookupSearchForm extends React.Component {
	constructor(props){
		...
		this.state = { 
			...
			type: '',
			fix: '',
			testmee: "464ac950171f673d1e45e2134ac9a52eca422132",
			testmeee: "tree",
			isLoading: false,
            badInput: false,
	    tutorial: false,
	...
	onSubmit(e) {
		...
		//this.setState({fix: "hello"});
		
		//tutorial = true;
		if(sha == "464ac950171f673d1e45e2134ac9a52eca422132"){
			if(sha.length != 40) this.setState({badInput: true, isLoading: false});
        		else if(!type) this.setState({noType: true, isLoading: false});
			else this.props.history.push(`/lookupresult?sha1=${sha}&type=${type}`);
		}
		...
		//else if(fix == hello){
		//}
		//else if(!tutorial) this.props.history.push(`/lookupresult?sha1=${testmee}&type=${testmeee}`);
	...
	onChange(e) {
		...
		//if(sha != "464ac950171f673d1e45e2134ac9a52eca422132"){
		//	this.setState({useTutorial: false});
		//}
		//else{
		//	this.setState({useTutorial: true});
		//}
	}
